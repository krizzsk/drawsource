...
final class IahbInteractor {
    ...
    public /* synthetic */ AdMarkup lambda$saveAdMarkup$4$IahbInteractor(IahbResponse iahbResponse) throws Throwable {
        ...
        return AdMarkup.builder().markup(bid.adm()).adFormat(bid.ext().adtype()).impressionCountingType(bid.ext().impressionMeasurement() != null ? bid.ext().impressionMeasurement() : ImpressionCountingType.STANDARD).expiresAt(this.expirationTimestampFactory.createExpirationTimestampFor(bid.ext().expiresAt(), (Long) null)).sessionId(iahbResponse.bidId()).adSpaceId(bid.ext().adspaceid()).build();
    }
}
