...
class HttpCaller implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (URLUtil.isHttpUrl(uri) || URLUtil.isHttpsUrl(uri)) {
            ...
            Headers of = Headers.m30607of(httpURLConnection.getHeaderFields());
            ...
            try {
                return Response.builder().request(chain.request()).responseCode(responseCode).headers(of).mimeType(mimeType).connection(httpURLConnection).body(HttpBody.builder().source(new BufferedInputStream(httpURLConnection.getInputStream())).contentLength((long) contentLength).build()).encoding(contentEncoding).build();
            } catch (Exception e) {
