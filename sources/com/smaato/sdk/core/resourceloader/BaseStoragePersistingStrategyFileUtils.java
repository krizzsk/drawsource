...
public class BaseStoragePersistingStrategyFileUtils {
    ...
    public void clearExpiredResources(String str, int i) {
        ...
        for (File next : listResourceFiles(str, i)) {
            ...
            long j = 0;
            try {
                j = Long.parseLong(name.substring(name.indexOf(".") + 1));
    ...
    public void deleteOldCaches(String str, int i) {
        ...
        if (diskCachesParentDirectory.exists() && (listFiles = diskCachesParentDirectory.listFiles()) != null) {
            for (File file : listFiles) {
                if (file.isDirectory()) {
                    try {
                        if (i > Integer.parseInt(file.getName())) {
                            arrayList.add(file);
                        }
        ...
        while (it.hasNext()) {
            ...
            for (File deleteFile : (File[]) Objects.requireNonNull(file2.listFiles())) {
    ...
    public List<File> listResourceFiles(String str, int i) {
        ...
        Objects.requireNonNull(str);
        ArrayList arrayList = new ArrayList();
        File currentDiskCacheDirectory = getCurrentDiskCacheDirectory(str, i);
        if (currentDiskCacheDirectory.exists() && (listFiles = currentDiskCacheDirectory.listFiles()) != null) {
            for (File file : listFiles) {
