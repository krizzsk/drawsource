...
public class GoogleApiAvailability extends GoogleApiAvailabilityLight {
        ...
        if (i == 18) {
            ...
            if (DeviceProperties.isWearable(context)) {
                ...
                if (DeviceProperties.isWearableWithoutPlayStore(context)) {
                    style.addAction(C5692R.C5693drawable.common_full_open_on_phone, resources.getString(C5692R.string.common_open_on_phone), pendingIntent);
                } else {
                    style.setContentIntent(pendingIntent);
                }
            } else {
                style.setSmallIcon(17301642).setTicker(resources.getString(C5692R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setContentIntent(pendingIntent).setContentText(zae);
            }
            if (PlatformVersion.isAtLeastO()) {
    ...
    public final boolean zag(Activity activity, LifecycleFragment lifecycleFragment, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        Dialog zaa2 = zaa(activity, i, zag.zad(lifecycleFragment, getErrorResolutionIntent(activity, i, DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT_VALUE_D), 2), onCancelListener);
        if (zaa2 == null) {
            return false;
        }
    ...
    public Dialog getErrorDialog(Activity activity, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        return zaa(activity, i, zag.zab(activity, getErrorResolutionIntent(activity, i, DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT_VALUE_D), i2), onCancelListener);
    }
    ...
    public Dialog getErrorDialog(Fragment fragment, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        return zaa(fragment.requireContext(), i, zag.zac(fragment, getErrorResolutionIntent(fragment.requireContext(), i, DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT_VALUE_D), i2), onCancelListener);
    }
}
