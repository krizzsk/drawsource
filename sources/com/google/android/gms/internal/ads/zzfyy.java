...
abstract class zzfyy {
    ...
    public abstract zzfyw zza(byte[] bArr, int i) throws InvalidKeyException;
    ...
    public final void zzb(ByteBuffer byteBuffer, byte[] bArr, byte[] bArr2, byte[] bArr3) throws GeneralSecurityException {
        if (byteBuffer.remaining() >= bArr2.length + 16) {
            ...
            byte[] zza2 = zzfzb.zza(zze(bArr), zzf(bArr3, byteBuffer));
            byteBuffer.limit(byteBuffer.limit() + 16);
            byteBuffer.put(zza2);
            return;
        }
    ...
    public final byte[] zzc(ByteBuffer byteBuffer, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        if (byteBuffer.remaining() >= 16) {
            ...
            try {
                if (zzghh.zzb(zzfzb.zza(zze(bArr), zzf(bArr2, byteBuffer)), bArr3)) {
                    byteBuffer.position(position);
