...
final class CctTransportBackend implements TransportBackend {
    ...
    private static int getPackageVersionCode(Context context) {
        try {
            ...
            Logging.m16663e(LOG_TAG, "Unable to find version code for package", e);
            return -1;
        }
    }
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            for (EventInternal eventInternal2 : (List) entry.getValue()) {
                ...
                if (encoding.equals(Encoding.m16657of("proto"))) {
                    builder = LogEvent.protoBuilder(encodedPayload.getBytes());
                } else if (encoding.equals(Encoding.m16657of("json"))) {
                    builder = LogEvent.jsonBuilder(new String(encodedPayload.getBytes(), Charset.forName("UTF-8")));
                } else {
                    Logging.m16665w(LOG_TAG, "Received event of unsupported encoding %s. Skipping...", encoding);
                }
    ...
    public HttpResponse doSend(HttpRequest httpRequest) throws IOException {
        ...
        Logging.m16664i(LOG_TAG, "Making request to: %s", httpRequest.url);
        ...
        try {
            ...
            try {
                ...
                Logging.m16664i(LOG_TAG, "Status Code: %d", Integer.valueOf(responseCode));
                Logging.m16660d(LOG_TAG, "Content-Type: %s", (Object) httpURLConnection.getHeaderField("Content-Type"));
                Logging.m16660d(LOG_TAG, "Content-Encoding: %s", (Object) httpURLConnection.getHeaderField("Content-Encoding"));
            ...
            Logging.m16663e(LOG_TAG, "Couldn't open connection, returning with 500", e);
            return new HttpResponse(500, (URL) null, 0);
        } catch (EncodingException | IOException e2) {
            Logging.m16663e(LOG_TAG, "Couldn't encode request, returning with 400", e2);
    ...
    public BackendResponse send(BackendRequest backendRequest) {
        ...
        try {
            ...
            if (httpResponse.code == 200) {
                return BackendResponse.m16658ok(httpResponse.nextRequestMillis);
            }
            ...
            Logging.m16663e(LOG_TAG, "Could not make request to the backend", e);
            return BackendResponse.transientError();
        }
    }
    ...
    static /* synthetic */ HttpRequest lambda$send$0(HttpRequest httpRequest, HttpResponse httpResponse) {
        ...
        Logging.m16660d(LOG_TAG, "Following redirect to: %s", (Object) httpResponse.redirectUrl);
