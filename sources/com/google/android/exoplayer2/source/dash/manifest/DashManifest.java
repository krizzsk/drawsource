...
public class DashManifest implements FilterableManifest<DashManifest> {
    ...
    public final long getPeriodDurationUs(int i) {
        return C5429C.msToUs(getPeriodDurationMs(i));
    }
    ...
    public final DashManifest copy(List<StreamKey> list) {
        ...
        while (true) {
            ...
            if (((StreamKey) linkedList.peek()).periodIndex != i) {
                ...
                arrayList.add(new Period(period.f14285id, period.startMs - j2, copyAdaptationSets(period.adaptationSets, linkedList), period.eventStreams));
            }
            i++;
        }
        ...
        do {
            ...
            do {
                ...
                if (!(poll.periodIndex == i && poll.groupIndex == i2)) {
                    arrayList.add(new AdaptationSet(adaptationSet.f14283id, adaptationSet.type, arrayList2, adaptationSet.accessibilityDescriptors, adaptationSet.essentialProperties, adaptationSet.supplementalProperties));
                }
                arrayList2.add(list2.get(poll.streamIndex));
                poll = linkedList.poll();
                break;
