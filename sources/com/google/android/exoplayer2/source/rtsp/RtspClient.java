...
final class RtspClient implements Closeable {
    ...
    private final class MessageSender {
        ...
        public void sendOptionsRequest(Uri uri, String str) {
            sendRequest(getRequestWithCommonHeaders(4, str, ImmutableMap.m20834of(), uri));
        }

        public void sendDescribeRequest(Uri uri, String str) {
            sendRequest(getRequestWithCommonHeaders(2, str, ImmutableMap.m20834of(), uri));
        }

        public void sendSetupRequest(Uri uri, String str, String str2) {
            sendRequest(getRequestWithCommonHeaders(10, str2, ImmutableMap.m20835of(RtspHeaders.TRANSPORT, str), uri));
        }

        public void sendPlayRequest(Uri uri, long j, String str) {
            sendRequest(getRequestWithCommonHeaders(6, str, ImmutableMap.m20835of("Range", RtspSessionTiming.getOffsetStartTimeTiming(j)), uri));
        }

        public void sendTeardownRequest(Uri uri, String str) {
            sendRequest(getRequestWithCommonHeaders(12, str, ImmutableMap.m20834of(), uri));
        }

        public void sendPauseRequest(Uri uri, String str) {
            sendRequest(getRequestWithCommonHeaders(5, str, ImmutableMap.m20834of(), uri));
        }
    ...
    private final class MessageListener implements RtspMessageChannel.MessageListener {
                ...
                try {
                    ...
                    switch (i) {
                        ...
                        case 6:
                            ...
                            if (str3 == null) {
                                immutableList = ImmutableList.m20815of();
                            } else {
        ...
        private void onPlayResponseReceived(RtspPlayResponse rtspPlayResponse) {
            ...
            RtspClient.this.playbackEventListener.onPlaybackStarted(C5429C.msToUs(rtspPlayResponse.sessionTiming.startTimeMs), rtspPlayResponse.trackTimingList);
        ...
        private void onPauseResponseReceived() {
            if (RtspClient.this.pendingSeekPositionUs != -9223372036854775807L) {
                RtspClient rtspClient = RtspClient.this;
                rtspClient.startPlayback(C5429C.usToMs(rtspClient.pendingSeekPositionUs));
            }
        }
    }
