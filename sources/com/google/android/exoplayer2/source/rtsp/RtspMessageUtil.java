...
final class RtspMessageUtil {
    ...
    private static final String CRLF = new String(new byte[]{Ascii.f18296CR, 10});
    public static final long DEFAULT_RTSP_TIMEOUT_MS = 60000;

    /* renamed from: LF */
    private static final String f14286LF = new String(new byte[]{10});
    ...
    public static byte[] convertMessageToByteArray(List<String> list) {
        return Joiner.m20785on(CRLF).join((Iterable<?>) list).getBytes(RtspMessageChannel.CHARSET);
    }
    ...
    public static RtspResponse parseResponse(List<String> list) {
        ...
        return new RtspResponse(parseInt, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m20785on(CRLF).join((Iterable<?>) list.subList(indexOf + 1, list.size())));
    }
    ...
    public static RtspRequest parseRequest(List<String> list) {
        ...
        Assertions.checkArgument(z);
        return new RtspRequest(parse, parseMethodString, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m20785on(CRLF).join((Iterable<?>) list.subList(indexOf + 1, list.size())));
    }
    ...
    public static String[] splitRtspMessageBody(String str) {
        return Util.split(str, str.contains(CRLF) ? CRLF : f14286LF);
    }
