...
public final class FrameworkMediaDrm implements ExoMediaDrm {
    ...
    private FrameworkMediaDrm(UUID uuid2) throws UnsupportedSchemeException {
        ...
        Assertions.checkArgument(!C5429C.COMMON_PSSH_UUID.equals(uuid2), "Use C.CLEARKEY_UUID instead");
        this.uuid = uuid2;
        this.mediaDrm = new MediaDrm(adjustUuid(uuid2));
        if (C5429C.WIDEVINE_UUID.equals(uuid2) && needsForceWidevineL3Workaround()) {
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws NotProvisionedException, DeniedByServerException {
        if (C5429C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    public FrameworkMediaCrypto createMediaCrypto(byte[] bArr) throws MediaCryptoException {
        return new FrameworkMediaCrypto(adjustUuid(this.uuid), bArr, Util.SDK_INT < 21 && C5429C.WIDEVINE_UUID.equals(this.uuid) && "L3".equals(getPropertyString("securityLevel")));
    }
    ...
    private static DrmInitData.SchemeData getSchemeData(UUID uuid2, List<DrmInitData.SchemeData> list) {
        boolean z;
        if (!C5429C.WIDEVINE_UUID.equals(uuid2)) {
            return list.get(0);
        }
    ...
    private static UUID adjustUuid(UUID uuid2) {
        return (Util.SDK_INT >= 27 || !C5429C.CLEARKEY_UUID.equals(uuid2)) ? uuid2 : C5429C.COMMON_PSSH_UUID;
    }
    ...
    private static byte[] adjustRequestInitData(java.util.UUID r2, byte[] r3) {
        /*
            java.util.UUID r0 = com.google.android.exoplayer2.C5429C.PLAYREADY_UUID
        ...
        L_0x001a:
            ...
            java.util.UUID r0 = com.google.android.exoplayer2.C5429C.WIDEVINE_UUID
            boolean r0 = r0.equals(r2)
    ...
    private static String adjustRequestMimeType(UUID uuid2, String str) {
        return (Util.SDK_INT >= 26 || !C5429C.CLEARKEY_UUID.equals(uuid2) || (!"video/mp4".equals(str) && !"audio/mp4".equals(str))) ? str : "cenc";
    }

    private static byte[] adjustRequestData(UUID uuid2, byte[] bArr) {
        return C5429C.CLEARKEY_UUID.equals(uuid2) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] bArr) {
        ...
        if (readLittleEndianShort == 1 && readLittleEndianShort2 == 1) {
            ...
            if (indexOf == -1) {
                Log.m16696w(TAG, "Could not find the </DATA> tag. Skipping LA_URL workaround.");
            }
        ...
        Log.m16694i(TAG, "Unexpected record count or type. Skipping LA_URL workaround.");
        return bArr;
    }
}
