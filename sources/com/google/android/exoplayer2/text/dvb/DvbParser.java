...
final class DvbParser {
    ...
    private static final byte[] defaultMap2To4 = {0, 7, 8, Ascii.f18305SI};
    ...
    private static void parseSubtitlingSegment(ParsableBitArray parsableBitArray, SubtitleService subtitleService2) {
        ...
        if (readBits3 * 8 > parsableBitArray.bitsLeft()) {
            Log.m16696w(TAG, "Data field length exceeds limit");
        ...
        switch (readBits) {
            ...
            case 17:
                ...
                if (readBits2 == subtitleService2.subtitlePageId && pageComposition2 != null) {
                    ...
                    if (pageComposition2.state == 0 && (regionComposition = subtitleService2.regions.get(parseRegionComposition.f14289id)) != null) {
                        parseRegionComposition.mergeFrom(regionComposition);
                    }
                    subtitleService2.regions.put(parseRegionComposition.f14289id, parseRegionComposition);
                    break;
                }
            case 18:
                if (readBits2 != subtitleService2.subtitlePageId) {
                    if (readBits2 == subtitleService2.ancillaryPageId) {
                        ...
                        subtitleService2.ancillaryCluts.put(parseClutDefinition.f14287id, parseClutDefinition);
                        break;
                    }
                } else {
                    ClutDefinition parseClutDefinition2 = parseClutDefinition(parsableBitArray, readBits3);
                    subtitleService2.cluts.put(parseClutDefinition2.f14287id, parseClutDefinition2);
                    break;
                }
                break;
            case 19:
                if (readBits2 != subtitleService2.subtitlePageId) {
                    if (readBits2 == subtitleService2.ancillaryPageId) {
                        ...
                        subtitleService2.ancillaryObjects.put(parseObjectData.f14288id, parseObjectData);
                        break;
                    }
                } else {
                    ObjectData parseObjectData2 = parseObjectData(parsableBitArray);
                    subtitleService2.objects.put(parseObjectData2.f14288id, parseObjectData2);
                    break;
                }
                break;
            case 20:
