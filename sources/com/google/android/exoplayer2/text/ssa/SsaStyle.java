...
final class SsaStyle {
    ...
    public static SsaStyle fromStyleLine(String str, Format format) {
        ...
        if (split.length != format2.length) {
            Log.m16696w(TAG, Util.formatInvariant("Skipping malformed 'Style:' line (expected %s values, found %s): '%s'", Integer.valueOf(format2.length), Integer.valueOf(split.length), str2));
            return null;
        }
        try {
    ...
    public static int parseAlignment(String str) {
        ...
        Log.m16696w(TAG, valueOf.length() != 0 ? "Ignoring unknown alignment: ".concat(valueOf) : new String("Ignoring unknown alignment: "));
        return -1;
    }
