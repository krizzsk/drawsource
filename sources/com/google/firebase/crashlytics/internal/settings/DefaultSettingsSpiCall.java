...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        if (z) {
            try {
                ...
                logger2.mo52101d("Requesting settings from " + this.url);
                Logger logger3 = this.logger;
                logger3.mo52109v("Settings query params were: " + queryParamsFor);
                return handleResponse(applyHeadersTo.execute());
            } catch (IOException e) {
                this.logger.mo52104e("Settings request failed.", e);
                return null;
            }
        } else {
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger2 = this.logger;
        logger2.mo52109v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger3 = this.logger;
        logger3.mo52103e("Settings request failed; (status: " + code + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger2 = this.logger;
            logger2.mo52112w("Failed to parse settings JSON from " + this.url, e);
            Logger logger3 = this.logger;
            logger3.mo52111w("Settings response " + str);
            return null;
        }
    }
