...
class CrashlyticsUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable th) {
        ...
        try {
            if (shouldRecordUncaughtException(thread, th)) {
                ...
                Logger.getLogger().mo52101d("Uncaught exception will not be recorded by Crashlytics.");
            }
        } catch (Exception e) {
            Logger.getLogger().mo52104e("An error occurred in the uncaught exception handler", e);
        } catch (Throwable th2) {
            Logger.getLogger().mo52101d("Completed exception processing. Invoking default exception handler.");
    ...
    private boolean shouldRecordUncaughtException(Thread thread, Throwable th) {
        if (thread == null) {
            Logger.getLogger().mo52103e("Crashlytics will not record uncaught exception; null thread");
            return false;
        } else if (th == null) {
            Logger.getLogger().mo52103e("Crashlytics will not record uncaught exception; null throwable");
            return false;
        } else if (!this.nativeComponent.hasCrashDataForCurrentSession()) {
            return true;
        } else {
            Logger.getLogger().mo52101d("Crashlytics will not record uncaught exception; native crash exists for session.");
            return false;
        }
    }
}
