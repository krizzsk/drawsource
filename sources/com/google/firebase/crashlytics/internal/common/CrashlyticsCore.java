...
public class CrashlyticsCore {
        ...
        if (isBuildIdValid(appData.buildId, CommonUtils.getBooleanResourceValue(this.context, CRASHLYTICS_REQUIRE_BUILD_ID, true))) {
            ...
            try {
                ...
                if (!didPreviousInitializationFail || !CommonUtils.canTryConnection(this.context)) {
                    Logger.getLogger().mo52101d("Successfully configured exception handler.");
                    return true;
                }
                Logger.getLogger().mo52101d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
                finishInitSynchronously(settingsProvider2);
                return false;
            } catch (Exception e) {
                Logger.getLogger().mo52104e("Crashlytics was not started due to an exception during initialization", e);
    ...
    public Task<Void> doBackgroundInitialization(SettingsProvider settingsProvider) {
        ...
        try {
            ...
            if (!settingsProvider.getSettingsSync().featureFlagData.collectReports) {
                Logger.getLogger().mo52101d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settingsProvider)) {
                Logger.getLogger().mo52111w("Previous sessions could not be finalized.");
            }
            ...
            Logger.getLogger().mo52104e("Crashlytics encountered a problem during asynchronous initialization.", e);
    ...
    public void logFatalException(Throwable th) {
        ...
        logger.mo52101d("Recorded on-demand fatal events: " + this.onDemandCounter.getRecordedOnDemandExceptions());
        Logger logger2 = Logger.getLogger();
        logger2.mo52101d("Dropped on-demand fatal events: " + this.onDemandCounter.getDroppedOnDemandExceptions());
    ...
    private void finishInitSynchronously(final SettingsProvider settingsProvider) {
        ...
        Logger.getLogger().mo52101d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            submit.get(4, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Logger.getLogger().mo52104e("Crashlytics was interrupted during initialization.", e);
        } catch (ExecutionException e2) {
            Logger.getLogger().mo52104e("Crashlytics encountered a problem during initialization.", e2);
        } catch (TimeoutException e3) {
            Logger.getLogger().mo52104e("Crashlytics timed out during initialization.", e3);
        }
    }
    ...
    public void markInitializationStarted() {
        ...
        Logger.getLogger().mo52109v("Initialization marker file was created.");
    }
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() {
            public Boolean call() throws Exception {
                try {
                    ...
                    if (!remove) {
                        Logger.getLogger().mo52111w("Initialization marker file was not properly removed.");
                    }
                    return Boolean.valueOf(remove);
                } catch (Exception e) {
                    Logger.getLogger().mo52104e("Problem encountered deleting Crashlytics initialization marker.", e);
                    return false;
                }
            }
        });
    ...
    static boolean isBuildIdValid(String str, boolean z) {
        if (!z) {
            Logger.getLogger().mo52109v("Configured not to require a build ID.");
            return true;
