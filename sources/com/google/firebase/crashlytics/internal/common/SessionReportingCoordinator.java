...
public class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager2, UserMetadata userMetadata) {
        ...
        if (findRelevantApplicationExitInfo == null) {
            Logger logger = Logger.getLogger();
            logger.mo52109v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        logger2.mo52101d("Persisting anr for session " + str);
    ...
    public void finalizeSessionWithNativeEvent(String str, List<NativeSessionFile> list) {
        Logger.getLogger().mo52101d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
    ...
    private CrashlyticsReport.Session.Event addLogsAndCustomKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager2, UserMetadata userMetadata) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().mo52109v("No log data to include with this event.");
        }
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52101d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            File reportFile = result.getReportFile();
            if (reportFile.delete()) {
                Logger logger2 = Logger.getLogger();
                logger2.mo52101d("Deleted report file: " + reportFile.getPath());
                return true;
            }
            Logger logger3 = Logger.getLogger();
            logger3.mo52111w("Crashlytics could not delete report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().mo52112w("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private static List<CrashlyticsReport.CustomAttribute> getSortedCustomAttributes(Map<String, String> map) {
        ...
        Collections.sort(arrayList, C7380x3e2a505a.INSTANCE);
        return arrayList;
    }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52111w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
