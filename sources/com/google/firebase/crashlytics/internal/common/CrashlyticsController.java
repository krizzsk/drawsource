...
class CrashlyticsController {
    ...
    public synchronized void handleUncaughtException(SettingsProvider settingsProvider2, Thread thread, Throwable th, boolean z) {
        ...
        logger.mo52101d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        ...
        try {
            Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() {
                public Task<Void> call() throws Exception {
                    ...
                    if (access$100 == null) {
                        Logger.getLogger().mo52103e("Tried to write a fatal exception while no session was open.");
                    ...
                    return settingsProvider3.getSettingsAsync().onSuccessTask(executor, new SuccessContinuation<Settings, Void>() {
                        public Task<Void> then(Settings settings) throws Exception {
                            String str = null;
                            if (settings == null) {
                                Logger.getLogger().mo52111w("Received null app settings, cannot send reports at crash time.");
                                return Tasks.forResult(null);
                            }
            ...
            Logger.getLogger().mo52103e("Cannot send reports. Timed out while fetching settings.");
        } catch (Exception e) {
            Logger.getLogger().mo52104e("Error handling uncaught exception", e);
        }
    }

    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().mo52101d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().mo52101d("Automatic data collection is disabled.");
        Logger.getLogger().mo52109v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().mo52101d("Waiting for send/deleteUnsentReports to be called.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().mo52109v("Found previous crash marker.");
    ...
    public Task<Boolean> checkForUnsentReports() {
        ...
        Logger.getLogger().mo52111w("checkForUnsentReports should only be called once per execution.");
    ...
    public Task<Void> submitAllReports(final Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().mo52109v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(null);
        }
        Logger.getLogger().mo52109v("Crash reports are available to be sent.");
                ...
                return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() {
                    public Task<Void> call() throws Exception {
                        if (!bool.booleanValue()) {
                            Logger.getLogger().mo52109v("Deleting cached crash reports...");
                            CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                            CrashlyticsController.this.reportingCoordinator.removeAllReports();
                            CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                            return Tasks.forResult(null);
                        }
                        Logger.getLogger().mo52101d("Sending cached crash reports...");
                        ...
                        return task.onSuccessTask(executor, new SuccessContinuation<Settings, Void>() {
                            public Task<Void> then(Settings settings) throws Exception {
                                if (settings == null) {
                                    Logger.getLogger().mo52111w("Received null app settings at app startup. Cannot send cached reports");
                                    return Tasks.forResult(null);
                                }
        ...
        this.backgroundWorker.submit((Runnable) new Runnable() {
            public void run() {
                if (!CrashlyticsController.this.isHandlingException()) {
                    ...
                    if (access$100 == null) {
                        Logger.getLogger().mo52111w("Tried to write a non-fatal exception while no session was open.");
                    } else {
    ...
    public void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider2 == null) {
            Logger.getLogger().mo52111w("settingsProvider not set");
        } else {
    ...
    public void setCustomKey(String str, String str2) {
        try {
            ...
            if (context2 == null || !CommonUtils.isAppDebuggable(context2)) {
                Logger.getLogger().mo52103e("Attempting to set custom attribute with null key, ignoring.");
                return;
            }
            throw e;
        }
    ...
    public boolean finalizeSessions(SettingsProvider settingsProvider2) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().mo52111w("Skipping session finalization because a crash has already occurred.");
            return Boolean.FALSE.booleanValue();
        }
        Logger.getLogger().mo52109v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider2);
            Logger.getLogger().mo52109v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().mo52104e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str) {
        ...
        Logger logger = Logger.getLogger();
        logger.mo52101d("Opening a new session with ID " + str);
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger logger = Logger.getLogger();
        logger.mo52109v("Finalizing native report for session " + str);
        NativeSessionFileProvider sessionFileProvider = this.nativeComponent.getSessionFileProvider(str);
        File minidumpFile = sessionFileProvider.getMinidumpFile();
        if (minidumpFile == null || !minidumpFile.exists()) {
            Logger logger2 = Logger.getLogger();
            logger2.mo52111w("No minidump data found for session " + str);
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().mo52111w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().mo52101d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().mo52112w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File next : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.mo52111w("Could not parse app exception timestamp from file " + next.getName());
            }
            next.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().mo52111w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().mo52101d("Logging app exception event to Firebase Analytics");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52109v("No ApplicationExitInfo available. Session: " + str);
            return;
        }
        Logger logger2 = Logger.getLogger();
        logger2.mo52109v("ANR feature enabled, but device is API " + Build.VERSION.SDK_INT);
    }
}
