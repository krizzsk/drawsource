...
class KeysMap {
    ...
    public synchronized boolean setKey(String str, String str2) {
        ...
        if (this.keys.size() >= this.maxEntries) {
            if (!this.keys.containsKey(sanitizeKey)) {
                ...
                logger.mo52111w("Ignored entry \"" + str + "\" when adding custom keys. Maximum allowable: " + this.maxEntries);
                return false;
            }
        }
    ...
    public synchronized void setKeys(Map<String, String> map) {
        ...
        if (i > 0) {
            Logger logger = Logger.getLogger();
            logger.mo52111w("Ignored " + i + " entries when adding custom keys. Maximum allowable: " + this.maxEntries);
        }
    }
