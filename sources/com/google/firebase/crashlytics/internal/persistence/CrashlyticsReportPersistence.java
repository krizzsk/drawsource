...
public class CrashlyticsReportPersistence {
    ...
    private static final FilenameFilter EVENT_FILE_FILTER = C7448x34929274.INSTANCE;
    private static final String EVENT_FILE_NAME_PREFIX = "event";
    private static final int EVENT_NAME_LENGTH = 15;
    private static final Comparator<? super File> LATEST_SESSION_ID_FIRST_COMPARATOR = C7447xde5b7e13.INSTANCE;
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().mo52101d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            logger.mo52102d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52112w("Could not persist event for session " + str, e);
        }
        trimEvents(str, i);
    }
    ...
    public void finalizeReports(String str, long j) {
        for (String next : capAndGetOpenSessions(str)) {
            Logger logger = Logger.getLogger();
            logger.mo52109v("Finalizing report for session " + next);
    ...
    public void finalizeSessionWithNativeEvent(String str, CrashlyticsReport.FilesPayload filesPayload) {
        ...
        Logger logger = Logger.getLogger();
        logger.mo52101d("Writing native session report for " + str + " to file: " + sessionFile);
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File next : allFinalizedReportFiles) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.mo52112w("Could not load report file " + next + "; deleting", e);
                next.delete();
            }
        }
        return arrayList;
    ...
    private SortedSet<String> capAndGetOpenSessions(String str) {
        ...
        while (openSessionIds.size() > 8) {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52101d("Removing session over cap: " + last);
    ...
    private void synthesizeReport(String str, long j) {
        ...
        if (sessionFiles.isEmpty()) {
            Logger.getLogger().mo52109v("Session " + str + " has no events.");
            return;
        }
        ...
        while (true) {
            ...
            while (true) {
                ...
                try {
                    ...
                    Logger.getLogger().mo52112w("Could not add event to report for " + next, e);
                }
            }
        }
        if (arrayList.isEmpty()) {
            Logger.getLogger().mo52111w("Could not parse event files for session " + str);
            return;
        }
    ...
    private void synthesizeNativeReportFile(File file, CrashlyticsReport.FilesPayload filesPayload, String str) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52112w("Could not synthesize final native report file for " + file, e);
        }
    }
    ...
    private void synthesizeReportFile(File file, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo52112w("Could not synthesize final report file for " + file, e);
        }
    }
    ...
    private int trimEvents(String str, int i) {
        List<File> sessionFiles = this.fileStore.getSessionFiles(str, C7446x19b0fc71.INSTANCE);
        Collections.sort(sessionFiles, C7445x6b11374a.INSTANCE);
