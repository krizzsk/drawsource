...
final class WavHeaderReader {
    ...
    public static void skipToData(ExtractorInput extractorInput, WavHeader wavHeader) throws IOException, InterruptedException {
        ...
        while (peek.f22107id != Util.getIntegerCodeForString("data")) {
            Log.w(TAG, "Ignoring unknown WAV chunk: " + peek.f22107id);
            long j = peek.size + 8;
            if (peek.f22107id == Util.getIntegerCodeForString("RIFF")) {
                j = 12;
            }
            if (j <= 2147483647L) {
                ...
                throw new ParserException("Chunk is too large (~2GB+) to skip; id: " + peek.f22107id);
            }
        }
